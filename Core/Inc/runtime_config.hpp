...
struct ModbusDeviceCfg {
    ...
    // Считается в РЕАЛЬНЫХ опросах этого датчика (не в тиках главного цикла) —
    // т.е. если poll_interval_polls=5 (опрос раз в 5 тиков) и avg_window_polls=60,
    // усреднение накопится за 60 реальных измерений = 300 тиков главного цикла.
    // 1 = без усреднения, в backup пишется каждое отдельное измерение (обратная
    // совместимость с поведением до Этапа 8).
