...
struct ModbusDeviceCfg {
    ...
    // Этап 7: индивидуальный интервал ОПРОСА датчика (не отправки!), в миллисекундах.
    // Ранее (Этап 7 v1) хранился как множитель тиков главного цикла
    // (poll_interval_polls, дискретность = Cfg().poll_interval_sec, обычно 5 сек).
    // Теперь — реальные миллисекунды: гейтинг в SensorReaderExt::pollRtuPorts()
    // сравнивает (HAL_GetTick() - m_lastPollTickMs[channel_idx]) с этим значением,
    // не завязываясь на дискретность главного цикла. 0 или значение меньше периода
    // главного цикла = опрос на каждой итерации (обратная совместимость).
    ...
    // Этап 8: окно усреднения ПЕРЕД записью в backup_ch{N}.jsn.
    // Считается в РЕАЛЬНЫХ опросах этого датчика (в количестве измерений, НЕ в
    // миллисекундах) — т.е. если poll_interval_ms=5000 (опрос раз в 5 сек) и
    // avg_window_polls=60, усреднение накопится за 60 реальных измерений = 300 сек.
    // 1 = без усреднения, в backup пишется каждое отдельное измерение (обратная
    // совместимость с поведением до Этапа 8). Единица измерения не меняется —
    // только poll_interval переведён из тиков главного цикла в миллисекунды.
    // Аналог ocean-station: value_buffer[metric_id] копится N сэмплов перед send,
    // но здесь усреднение происходит ДО записи в бэкап, а не перед отправкой —
    // отправка (App::retransmitBackup()) просто перегоняет уже усреднённые записи.
    uint16_t avg_window_polls = 1;
};
